...
static void test_return_type() {
  ...
  {
    std::same_as<cr::utc_time<cr::nanoseconds>> decltype(auto) _ =
        cr::utc_clock::from_sys(cr::sys_time<cr::nanoseconds>{0ns});
  }
  {
    std::same_as<cr::utc_time<cr::microseconds>> decltype(auto) _ =
        cr::utc_clock::from_sys(cr::sys_time<cr::microseconds>{0us});
  }
  {
    std::same_as<cr::utc_time<cr::milliseconds>> decltype(auto) _ =
        cr::utc_clock::from_sys(cr::sys_time<cr::milliseconds>{0ms});
  }

  {
    std::same_as<cr::utc_time<cr::seconds>> decltype(auto) _ =
